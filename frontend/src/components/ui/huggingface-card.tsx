      ...
      >
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-lg font-semibold text-gray-200 line-clamp-1" aria-label={`Model: ${name}`}>
              {name}
            </h3>
            <span className="text-xs px-2 py-1 rounded-full bg-zinc-800 text-gray-400 ml-2 flex items-center gap-1">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            {metrics.tags?.slice(0, 2).map((tag, index) => (
              <span key={index} className="text-xs px-2 py-1 rounded-full bg-zinc-800 text-gray-400 flex items-center gap-1">
          ...
          <div className="mb-4 flex-1">
            <AnimatePresence>
              <motion.p
                initial={false}
                animate={{ height: isExpanded ? "auto" : "3.5em" }}
                className={cn(
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {metrics.likes !== undefined && (
              <div className="bg-zinc-800/50 rounded-lg p-2 flex items-center gap-1.5" aria-label={`${metrics.likes} likes`}>
                ...
                <span className="text-sm font-medium text-gray-200">{metrics.likes.toLocaleString()}</span>
            ...
            {metrics.downloads !== undefined && (
              <div className="bg-zinc-800/50 rounded-lg p-2 flex items-center gap-1.5" aria-label={`${metrics.downloads} downloads`}>
                ...
                <span className="text-sm font-medium text-gray-200">{metrics.downloads.toLocaleString()}</span>
            ...
            {metrics.contributors !== undefined && (
              <div className="bg-zinc-800/50 rounded-lg p-2 flex items-center gap-1.5" aria-label={`${metrics.contributors} contributors`}>
                ...
                <span className="text-sm font-medium text-gray-200">{metrics.contributors.toLocaleString()}</span>
          ...
          <motion.a
            ...
            className="w-full bg-primary/10 hover:bg-primary/20 text-primary rounded-lg py-2 px-4 text-sm font-medium flex items-center justify-center gap-1.5 transition-colors"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
